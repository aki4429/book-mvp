        ...
        <nav class="flex-1 px-2 py-4 space-y-2">
            <a href="{{ route('dashboard') }}"
               class="flex items-center px-3 py-2 rounded-lg
                ...
                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor"><use href="#home-icon"/></svg>
                <span>ダッシュボード</span>
            </a>
            <a href="{{ route('reservations.index') }}"
               class="flex items-center px-3 py-2 rounded-lg hover:bg-gray-200 {{ request()->routeIs('reservations.*') ? 'bg-gray-200 font-semibold' : '' }}">
                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor"><use href="#calendar-icon"/></svg>
                <span>予約管理</span>
            </a>
            <a href="{{ route('customers.index') }}"
               class="flex items-center px-3 py-2 rounded-lg hover:bg-gray-200 {{ request()->routeIs('customers.*') ? 'bg-gray-200 font-semibold' : '' }}">
                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor"><use href="#users-icon"/></svg>
                <span>顧客管理</span>
            </a>
