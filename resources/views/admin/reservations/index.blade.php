  ...
  <div class="overflow-x-auto bg-white shadow rounded-lg">
    <table class="min-w-full divide-y divide-gray-200">
      ...
      <tbody class="divide-y divide-gray-100 text-sm">
          ...
          <tr>
            ...
            <td class="px-4 py-2">
              @php
                ...
                $color = $statusColors[$r->status] ?? 'bg-gray-100 text-gray-800';
              @endphp

              <span class="px-2 py-1 rounded {{ $color }}">{{ $r->status ?? '不明' }}</span>
            </td>

            <td class="px-4 py-2">{{ $r->created_at->format('Y-m-d') }}</td>
            <td class="px-4 py-2">
              <a href="{{ route('admin.reservations.show', $r) }}" class="text-blue-600 hover:underline">詳細</a>
        ...
        @empty
          <tr>
            <td colspan="5" class="px-4 py-8 text-center text-gray-500">予約がありません</td>
          </tr>
        @endforelse
      </tbody>
    </table>
