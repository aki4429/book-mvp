...
@section('page-title', '曜日ベース予約枠 一括登録')

@section('body')
  <div class="max-w-4xl mx-auto">
    <h2 class="text-2xl font-bold mb-6">曜日ベース予約枠 一括登録</h2>

    <form method="POST" action="{{ route('admin.timeslots.bulkStore') }}" id="bulk-form">
      @csrf
        ...
        <div class="grid grid-cols-7 gap-2">
          @foreach (['mon' => '月', 'tue' => '火', 'wed' => '水', 'thu' => '木', 'fri' => '金', 'sat' => '土', 'sun' => '日'] as $key => $label)
            <label class="flex items-center justify-center p-2 border rounded hover:bg-gray-50 cursor-pointer">
              <input type="checkbox" name="days[]" value="{{ $key }}" class="mr-2">
      ...
      <div class="mb-6 p-4 border rounded-lg bg-blue-50">
        <div class="flex justify-between items-center mb-3">
          ...
          <a href="{{ route('admin.presets.index') }}" class="text-sm text-blue-600 hover:text-blue-800">
            プリセット管理 →
          </a>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
          @forelse($presets as $preset)
            <button type="button" class="preset-btn bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600"
              data-preset='@json($preset->time_slots)' title="{{ $preset->description }}">
              {{ $preset->name }}
            </button>
          @empty
            <div class="col-span-full text-gray-500 text-center">
              プリセットがありません。
              <a href="{{ route('admin.presets.create') }}" class="text-blue-600 hover:text-blue-800">新規作成</a>
            </div>
          @endforelse
        </div>
      </div> <!-- 送信ボタン -->
