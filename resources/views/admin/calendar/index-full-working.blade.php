...
<body class="bg-gray-50">
    ...
    <script>
        ...
        function changeMonth(direction) {
            ...
            fetch('/admin/calendar/change-month?' + params.toString(), {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                if (data.success && data.html) {
                    document.querySelector('.calendar-grid').innerHTML = data.html;
                    updateCalendarDisplay();
                }
            })
            .catch(error => {
        ...
        function loadDaySlots(date) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('daySlots').innerHTML = data.html || '<p>この日に時間枠はありません。</p>';
                    document.getElementById('dayDetails').style.display = 'block';
                } else {
                    document.getElementById('selectedDate').textContent = data.date + 'の詳細';
                    document.getElementById('daySlots').innerHTML = '<p class="text-red-500">データの読み込みに失敗しました。</p>';
                    document.getElementById('dayDetails').style.display = 'block';
                }
            })
            .catch(error => {
                console.error('Error loading day slots:', error);
                document.getElementById('selectedDate').textContent = date + 'の詳細';
                document.getElementById('daySlots').innerHTML = '<p class="text-red-500">エラーが発生しました。</p>';
                document.getElementById('dayDetails').style.display = 'block';
            });
        }
