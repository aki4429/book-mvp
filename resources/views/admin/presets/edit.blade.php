...
@section('body')
  <div class="max-w-4xl mx-auto">
    <div class="flex items-center mb-6">
      ...
      <h2 class="text-2xl font-bold">プリセット編集: {{ $preset->name }}</h2>
    </div>

    <form method="POST" action="{{ route('admin.presets.update', $preset) }}">
      @csrf
      @method('PUT')
      ...
      <div class="mb-6 p-4 border rounded-lg">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $preset->name) }}"
          ...
          <div>
            ...
            <input type="text" name="description" id="description"
              value="{{ old('description', $preset->description) }}" class="w-full border rounded px-3 py-2">
            @error('description')
              <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="mt-4">
          <label class="flex items-center">
            <input type="checkbox" name="is_active" value="1"
              {{ old('is_active', $preset->is_active) ? 'checked' : '' }} class="mr-2">
            このプリセットを有効にする
          </label>
        </div>
      </div>
      ...
      <div class="mb-6 p-4 border rounded-lg">
        ...
        <div id="time-slots-container">
          @foreach (old('time_slots', $preset->time_slots) as $index => $slot)
            <div class="time-slot-row mb-4 p-3 border rounded bg-gray-50">
              <div class="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input type="time" name="time_slots[{{ $index }}][end_time]" value="{{ $slot['end_time'] }}"
                    class="w-full border rounded px-3 py-2" required>
                </div>
                <div>
                  <label class="block text-sm font-medium mb-1">定員</label>
                  <input type="number" name="time_slots[{{ $index }}][capacity]" value="{{ $slot['capacity'] }}"
  ...
  @push('scripts')
    <script>
      let timeSlotIndex = {{ count(old('time_slots', $preset->time_slots)) }};
